...
void setup(){
  ...
  background(254);
    img.loadPixels(); 
    for (int x = 0; x < startSizeX; x+=incrementer) {
      for (int y = 0; y < startSizeY; y+=incrementer ) {
  ...
  //void update(int imagePos, int canvasPos) {
    void update(){
     ...
     if(xpos>=width || ypos>=height ||xpos<=0 || ypos<=0){
      ...
      float noise = map(pixels[canvasPos],0,255,-1,1);
    ...
    if(sin(angle)>0){
     ...
     ypos += velocity.y;
      ...
      if(velocity.x>speed_lim){
        velocity.x-=2;
     }else if(velocity.x<speed_lim){
        velocity.x+=2;
     }
     if(velocity.y>speed_lim){
        velocity.y-=2;
     }else if(velocity.y<speed_lim){
        velocity.y+=2;
     }
